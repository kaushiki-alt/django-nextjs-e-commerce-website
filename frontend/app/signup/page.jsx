...
const Page = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await run(() => authApi.post('/register/', data));
  ...
  return (
    <main className="flex items-center justify-center bg-bg-primary/100 min-h-screen p-4">
      <div className="w-full max-w-md p-8 space-y-6 bg-card-bg rounded-lg shadow-lg border border-card-border ">
        ...
        {success && <p className="text-green-500 text-center">Signup successful! Redirecting to signin...</p>}
            ...
            <>
              ...
              <div className="flex justify-between">
                <button
                  type="button"
                  onClick={handlePrev}
                  className="w-1/2 mr-2 flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-gray-600 hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors"
                >
                  Previous
                </button>
                <button
                  type="submit"
                  disabled={loading}
                  className="w-1/2 ml-2 flex justify-center py-2 px-4 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-accent hover:bg-accent-hover focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-accent transition-colors"
                >
        ...
        <p className="text-center text-sm text-text-muted">
          ...
          <a href="/signin" className="font-medium text-accent hover:underline">Sign In</a>
        </p>
      </div>
    </main>
  );
